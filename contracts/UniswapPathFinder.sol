...
library UniswapPathFinder {
    ...
    function _calculateSingleSwapV3(
        ...
        if (exactIn) {
            IQuoterV2.QuoteExactInputSingleParams memory data = 
                IQuoterV2.QuoteExactInputSingleParams(tokenIn, tokenOut, amount, fee, 0);
            
            try quoter.quoteExactInputSingle(data) returns (uint256 newAmount, uint160, uint32, uint256) {
                return newAmount;
            } catch {
                return 0;
        }
        } else {
            IQuoterV2.QuoteExactOutputSingleParams memory data = 
                IQuoterV2.QuoteExactOutputSingleParams(tokenIn, tokenOut, amount, fee, 0);
            
            try quoter.quoteExactOutputSingle(data) returns (uint256 newAmount, uint160, uint32, uint256) {
                return newAmount;
            } catch {
